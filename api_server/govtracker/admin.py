...
class GuestlistModelAdminForm(forms.ModelForm):
    pass
...
class GuestlistModelAdmin(admin.ModelAdmin):
    form = GuestlistModelAdminForm
#    list_filter = ('username') # lounge foriegn field
    search_fields = ('username__username', 'username__firstname', 'username__lastname', 'guest')
    list_display = ('username', 'guest')
    ...
    def get_actions(self, request):
        actions = super(GuestlistModelAdmin, self).get_actions(request)
   ...
   #    form = super(PackageModelAdmin, self).get_form(request, obj, **kwargs)
    ...
    def save_model(self, request, obj, form, change):
      ...
      #      obj.checkin = datetime.now()
       # obj.save()
        #aise Exception('title:' + str(form.cleaned_data['title']))
        #raise Exception(str(form.changed_data))
        raise Exception('Changes to guest list not allowed')
    ...
    def get_queryset(self, request):
        return super(GuestlistModelAdmin, self).get_queryset(request).order_by('username__firstname')
...
admin.site.register(Guestlist, GuestlistModelAdmin)
